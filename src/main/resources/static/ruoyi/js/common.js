...
//页面跳转方法——通用
function directPage(dataUrl){
	//新增tab页
	createMenuItem(dataUrl,"");
	//隐藏新增标签页，只保留iframe内容
	addTabHide(dataUrl);
}

//同一页面跳转，隐藏原页面内容
function addTabHide(dataId){
	var topWindow = $(window.parent.document);
	//移除当前新增tab标签
	$('.menuTab[data-id="' + dataId + '"]', topWindow).remove();
	
	//保留当前跳转页面，以便返回时保留原页面
	var dataId1 = window.frameElement.getAttribute('data-id');
	$('.menuTab[data-id="' + dataId1 + '"]', topWindow).addClass('active');
	$('.mainContent .RuoYi_iframe[data-id="' + dataId1 + '"]', topWindow).hide();
}
...
function rebackPage(){
	...
	//获取原页面地址
	var dataPanel = window.frameElement.getAttribute('data-panel');
	
	//返回，将原页面css属性设为显示
	$('.mainContent .RuoYi_iframe[data-id="' + dataPanel + '"]', topWindow).attr('style','');
...
function rebackPageFresh(){
	...
	$('.mainContent .RuoYi_iframe[data-id="' + dataPanel + '"]', topWindow).attr('style','');
	//刷新
	$('.mainContent .RuoYi_iframe[data-id="' + dataPanel + '"]', topWindow).attr('src',dataPanel).load(function () {
		$.modal.closeLoading(); 
    }) 
